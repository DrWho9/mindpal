...
function wrapRuntime() {
  ...
  const ytDirectoryCandidates = readFileSync(
    join(root, "src/data/yt-directory-candidates.json"),
    "utf8",
  );
  const ytDirectoryViews = readFileSync(
    join(root, "src/data/yt-directory-views.json"),
    "utf8",
  );
...
function patchOwnerUx(source) {
  ...
  if (!next.includes("DETERMINISTIC DEMO · NO LIVE AI") || !next.includes("mpCompanionDemo=")) {
