...
const checks = [
  ...
  [sw.includes('prefix:"mindpal-shell-v7"'), "service worker cache prefix was bumped so stale shells drop"],
  [sw.includes("vendor/pdfjs/pdf.min.js") && sw.includes("vendor/pdfjs/pdf.worker.min.js"), "service worker precaches the on-device PDF parser"],
  ...
  [js.includes("function mpYourBooksPage(") && js.includes("t===`Book`") && js.includes("Open your books"), "private Book route and Explore card are present"],
  [js.includes("mindpal-book-idb") && js.includes("mindpal-book-design-v1") && js.includes("mindpal-book-ambient-v1"), "Book storage keys from the old reader are present"],
  [js.includes("/mindpal/vendor/pdfjs/pdf.min.js") && js.includes("/mindpal/vendor/pdfjs/pdf.worker.min.js"), "PDF parser is same-origin for the Pages CSP"],
  [!js.includes("id:`Book`,label:`Book`"), "Book is not a sixth bottom tab"],
  ...
  [js.includes("formatDirectoryViews") && js.includes("groupDirectoryByCategory") && js.includes("mpYtDirectoryViews"), "YouTube directory groups by topic and reads the view snapshot"],
  [js.includes("View count unavailable"), "missing YouTube view counts are labelled, not guessed"],
  [js.includes("mp-yt-dir-category"), "YouTube directory renders category headings"],
