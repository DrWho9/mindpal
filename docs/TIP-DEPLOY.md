...
2. On the device that smokes tip, do **one** of:
   ...
   - **Hard refresh** with a new query: `https://<host>/mindpal/?cb=<new>` (this branch: `?cb=shell-v4`).
...
Bump the Workbox precache prefix when tip hashes change so old shells are not reused. `#36` shipped `mindpal-shell-v3`. This branch is **`mindpal-shell-v4`** (past that conflict). `registerSW.js` calls `reg.update()` and, on controller change, reloads with `?cb=shell-v4` if `cb` is missing.
